# PARMA-4, the PHITS-based Analytical Radiation Model in the Atmosphere (PARMA)

The functions in `subroutines.cpp` and the data tables in the `input/` directory are copied from
PARMA-4, a framework to compute the atmospheric cosmic-ray fluxes and dose rates. You can find
the original code and data at http://phits.jaea.go.jp/expacs. They are the work of Tatsuhiko Sato.

The licensing conditions of PARMA-4 are:
- Commercial use requires prior agreement with the Japan Atomic Energy Agency.
- Non-commercial use is allowed, provided that manuscripts based on PARMA cite both:
  1. T.Sato, "Analytical model for estimating terrestrial cosmic-ray fluxes nearly anytime and anywhere in the world; Extension of PARMA/EXPACS," _PLOS ONE_,  **10**(12): [e0144679](https://doi.org/10.1371/journal.pone.0144679) (2015)
  2. T.Sato, "Analytical model for estimating the zenith angle dependence of terrestrial cosmic ray fluxes," _PLOS ONE_, **11**(8): [e0160390](https://doi.org/10.1371/journal.pone.0160390) (2016)


The README from the C++ version of PARMA-4 follows.
